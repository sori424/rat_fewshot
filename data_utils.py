...
def load_rat_harder():
    train_words = []
    train_answers = []
    with open('data/rat/train_wl.jsonl','r') as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['Item']
            p = ''.join(myjson['Harder List'])
            
            train_answers.append(myjson['Answer'])
            train_words.append(q + '\n' + 'Choices: ' + p)

    test_words = []
    test_answers = []
    with open('data/rat/test_wl.jsonl', 'r') as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['Item']
            p = ''.join(myjson['Harder List'])

            test_answers.append(myjson['Answer'])
            test_words.append(q + '\n' + 'Choices: ' + p)

    return train_words, train_answers, test_words, test_answers


def load_rat_harder2():
    train_words = []
    train_answers = []
    with open('data/rat/train_wl.jsonl','r') as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['Item']
            p = ''.join(myjson['Harder Many List'])
...
def load_rat_correct():
    ...
    with open('data/rat/train_wl.jsonl','r') as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['Item']
            p = ''.join(myjson['Answer-related Many List'])
...
def load_rat():
    ...
    with open('data/rat/train_reason.jsonl', 'r') as json_file:
        json_list = list(json_file)

    for i in json_list:
        i = ast.literal_eval(i)
        q = ', '.join(list(map(str.strip, i['Item'].split('/'))))
        p = i['Answer']

        train_answers.append(p)
        train_words.append(q + '?')
        # train_words.append(p)

    test_words = []
    test_answers = []
    with open('data/rat/test_syn.jsonl','r') as f:
        for line in f:
            myjson = json.loads(line)
            q = ', '.join(list(map(str.strip, myjson['Item'].split('/'))))
...
# Rat reason chain w/ meta
def load_rat_reason_chain_meta():
...
# Rat reason chain wo meta
def load_rat_reason_chain_wo_meta():
...
def load_dataset(params):
    ...
    elif params['dataset'] == 'rat_harder2':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_harder2()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Let's pick the correct answer out of the choices.\n"
        params['q_prefix'] = 'Cue words: '
        params['a_prefix'] = 'Correct answer: '
    ...
    elif params['dataset'] == 'rat_correct':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_correct()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Let's pick the correct answer out of the choices.\n"
        params['q_prefix'] = 'Cue words: '
        params['a_prefix'] = 'Correct answer: '
        params['task_format'] = 'qa'
    ...
    elif params['dataset'] == 'rat_harder':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_harder()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Let's pick the correct answer out of the choices.\n"
        params['q_prefix'] = 'Cue words: '
        params['a_prefix'] = 'Correct answer: '
        params['task_format'] = 'qa'
    ...
    elif params['dataset'] == 'rat':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Give me the correct answer.\n"
        # params['prompt_prefix'] = "Let's find a fourth word that is related to all three of the following.\n"
        params['q_prefix'] = 'Q: What word is related to all three of the following, '
        # 'Cue words: '
        params['a_prefix'] = 'A: '
        params['task_format'] = 'qa'
        params['num_tokens_to_predict'] = 1
    ...
    elif params['dataset'] == 'rat_reason_template':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_reason()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Give me the answer and reason by using this template: A: The correct answer is {Answer}. Because {Reason}.\n"
        # params['prompt_prefix'] = "Let's find a fourth word that is related to all three of the following.\n"
        params['q_prefix'] = 'Q: What word is related to all three of the following, '
        # 'Cue words: '
        params['a_prefix'] = 'A: '
    ...
    elif params['dataset'] == 'rat_reason_chain_meta_template':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_reason_chain_meta()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Give me the reason and answer by using this template: A: Because {Reason}. The correct answer is {Answer}. \n"
        # params['prompt_prefix'] = "Let's find a fourth word that is related to all three of the following.\n"
        params['q_prefix'] = 'Q: What word is related to all three of the following, '
        # 'Cue words: '
        params['a_prefix'] = 'A: '
    ...
    elif params['dataset'] == 'rat_reason_wo_meta':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_reason_wo_meta()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Give me the answer and reason.\n"
        # params['prompt_prefix'] = "Let's find a fourth word that is related to all three of the following.\n"
        params['q_prefix'] = 'Q: What word is related to all three of the following, '
        # 'Cue words: '
        params['a_prefix'] = 'A: '
    ...
    elif params['dataset'] == 'rat_reason_chain_wo_meta':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_rat_reason_chain_wo_meta()
        params['prompt_prefix'] = "Let's do Remote Association Test. Each question presents three cue words that are linked by a fourth word, which is the correct answer. Give me a reason and the answer.\n"
        # params['prompt_prefix'] = "Let's find a fourth word that is related to all three of the following.\n"
        params['q_prefix'] = 'Q: What word is related to all three of the following, '
        # 'Cue words: '
        params['a_prefix'] = 'A: '
